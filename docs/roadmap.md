...
[ ] 1. 表达式规范化与简化：扁平化、去重、德摩根转换、常量折叠
[ ] 2. 表达式解析器与 DSL：支持括号、优先级、Tag 表达式语法, 支持 DSL 字符串直接解析为表达式，包含 `^tool_.*` 与 `` `tool_name` `` 的简写形式。使用 Lark 库实现 DSL 解析。
[ ] 3. 诊断信息：输出命中路径与拒绝原因，便于调试过滤逻辑
[ ] 4. 编译与缓存：注册阶段编译谓词并缓存，避免重复解析
[ ] 5. 批量评估接口：一条表达式对多工具的高效匹配
[ ] 6. 标签匹配扩展：命名空间、前缀匹配、别名与正则匹配
